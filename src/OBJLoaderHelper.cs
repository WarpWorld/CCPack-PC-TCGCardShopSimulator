...
public static class OBJLoaderHelper  {
    ...
    {
        // Attempt to find the default Standard shader
        Shader defaultShader = Shader.Find("Standard");

        // If the shader is not found, log an error and return a fallback material
        if (defaultShader == null)
        {
            Debug.LogError("Failed to find Standard shader. Please ensure it's included in the project.");
            return new Material(Shader.Find("Diffuse")); // Use another fallback shader like Diffuse if necessary
        }


        return new Material(defaultShader); // Create the material with the Standard shader
    }
